...
function findOffsets(
  ...
  if (start < 0) {
    throw new Error(
      `Sample dossier integrity error: highlight quote not found in source text.\nQuote: ${JSON.stringify(quote.slice(0, 80))}...`,
    );
  }
...
/**
 * Build the curated "Northgate Pharma" sample dossier under the given owner.
 *
 * Used by both the dev seed CLI (`prisma/seed.ts`) and the user-facing
 * "Load sample dossier" server action so the demo content has one source of
 * truth.
 *
 * Returns the new dossier's id so callers can redirect or report on it.
 */
export async function buildSampleDossier(
  client: PrismaClient,
  ownerId: string,
): Promise<{ dossierId: string }> {
  // ─── Tags ────────────────────────────────────────────────────────────────────
  // Tags are unique by name and shared across users; upsert so repeated
  // invocations don't collide.

  const tagRegulatory = await client.tag.upsert({
    where: { name: "regulatory" },
    update: {},
    create: { name: "regulatory" },
  });

  const tagFinancial = await client.tag.upsert({
    where: { name: "financial" },
    update: {},
    create: { name: "financial" },
  });

  const tagManufacturing = await client.tag.upsert({
  ...
  // ─── Dossier ─────────────────────────────────────────────────────────────────
  // Slug is uniqued per owner; suffix on collision so a user who loads the
  // sample twice gets two separate dossiers rather than an error.

  const baseSlug = "northgate-pharma-fda-warning";
  let slug = baseSlug;
  let n = 1;
  while (await client.dossier.findFirst({ where: { owner_id: ownerId, slug } })) {
    slug = `${baseSlug}-${n++}`;
  }

  const dossier = await client.dossier.create({
    data: {
      owner_id: ownerId,
...
We acknowledge receipt of your response dated November 30, 2023. After reviewing your response, we conclude it does not adequately address our concerns. Failure to promptly correct these violations may result in legal action without further notice, including, without limitation, seizure and injunction.`;
  ...
  const sourceFdaLetter = await client.source.create({
    data: {
...
Analysts at Morgan Stanley estimated the financial impact at between $40M and $120M depending on whether the FDA proceeds to a consent decree. The company's Raleigh facility accounts for approximately 34% of total revenue.`;
  ...
  const sourceReuters = await client.source.create({
    data: {
      dossier_id: dossier.id,
      type: "web_link",
      title:
...
Raymond Chu`;
  ...
  const sourceMemo = await client.source.create({
    data: {
      dossier_id: dossier.id,
      type: "internal_memo",
...
A: If the company had a prior 483 at the same site and documentation gaps weren't fully remediated, the probability of a consent decree is high. I'd put it above 50%. The financial range analysts are quoting — $40M to $120M — honestly, consent decrees for facilities this size can run higher if they include facility remediation requirements. I would not anchor to that lower number.`;
  ...
  const sourceExpertInterview = await client.source.create({
    data: {
      dossier_id: dossier.id,
      type: "pasted_text",
...
— David Harrington, CEO`,
  ...
  const hl1 = await client.highlight.create({
    data: {
  ...
  const hl2 = await client.highlight.create({
    data: {
      source_id: sourceFdaLetter.id,
      quote_text:
  ...
  const hl3 = await client.highlight.create({
    data: {
  ...
  const hl4 = await client.highlight.create({
    data: {
  ...
  const hl5 = await client.highlight.create({
    data: {
      source_id: sourceMemo.id,
      quote_text:
        "I am formally requesting executive sign-off on a CAPA within 30 days.",
  ...
  const hl6 = await client.highlight.create({
    data: {
  ...
  const hl7 = await client.highlight.create({
    data: {
      source_id: sourceExpertInterview.id,
  ...
  const claimSystemic = await client.claim.create({
    data: {
      dossier_id: dossier.id,
      statement:
  ...
  const claimKnowledge = await client.claim.create({
    data: {
      dossier_id: dossier.id,
      statement:
  ...
  const entityNorthgate = await client.entity.create({
    data: {
      dossier_id: dossier.id,
  ...
  const entityWei = await client.entity.create({
    data: {
      dossier_id: dossier.id,
      name: "Dr. Sandra Wei",
      type: "person",
  ...
  const entityChu = await client.entity.create({
    data: {
      dossier_id: dossier.id,
      name: "Raymond Chu",
      type: "person",
  ...
  const entityHarrington = await client.entity.create({
    data: {
      dossier_id: dossier.id,
  ...
  // ─── Mentions ─────────────────────────────────────────────────────────────────
  // Look up the unreviewed shareholder letter source by URL since we didn't
  // hold a reference to it (it has no highlights or claims of its own yet).

  const sourceShareholderLetter = await client.source.findFirstOrThrow({
    where: {
      dossier_id: dossier.id,
      url: "https://example.com/northgate-2023-annual-letter",
    },
